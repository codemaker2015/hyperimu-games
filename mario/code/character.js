...
function socketCommunication(){
    ...
    if ("WebSocket" in window) {
        ...
        ws = new WebSocket("ws://localhost:2055");
        // Let us open a web socket
        ws.onopen = function() {
            // Web Socket is connected, send data using send()
            ws.send('{ "type": "authentication", "moduleId": "mario", "moduleSecret": "qwerty" }');
            ws.send('{ "type":"setCapabilities", "kaiId": "default", "pyrData": true, "fingerShortcutData": true }');
...
Mario.Character.prototype.Move = function() {
    ...
    if(datas != ""){
        ...
        if(obj.type == "incomingData"){
            if(obj.data[0].type == "fingerShortcutData"){  
                if(Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S) || (obj.data[0].fingers[0] || obj.data[0].fingers[1] || obj.data[0].fingers[2] || obj.data[0].fingers[3]) || (this.JumpTime < 0 && !this.OnGround && !this.Sliding)){
                    if (this.JumpTime < 0) {
            ...
            if(obj.data[0].type == "pyrData"){
                if((Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.Left) || obj.data[0].roll < -40 )&& !this.Ducking){ //Moving backward
                    roll = obj.data[0].roll;
                    if (this.Facing === 1) {
                        this.Sliding = false;
                    }
                ...
                }else if((Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.Right) || obj.data[0].roll > 40) && !this.Ducking){ //Moving forward
                    roll = obj.data[0].roll;
                    if (this.Facing === -1) {
                        this.Sliding = false;
                    }
               ...
               if(obj.data[0].pitch < -30  || (this.JumpTime < 0 && !this.OnGround && !this.Sliding)){
                    if (this.JumpTime < 0) {
                        this.Xa = this.XJumpSpeed;
                        this.Ya = -this.JumpTime * this.YJumpSpeed;
                        this.JumpTime++;
            ...
            if ((!Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.Left) && !Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.Right) && !obj.data[0].roll > 50 && obj.data[0].roll < -50) || this.Ducking || this.Ya < 0 || this.OnGround) {
                this.Sliding = false;  
            }
        }
    }
